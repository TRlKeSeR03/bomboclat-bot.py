...
def get_ai_response(prompt, system_context, full_history):
    # 1. ÖNCELİK: GEMINI (Kota daha geniş ve daha zeki)
    if gemini_iterator:
        for _ in range(len(GEMINI_KEYS)):
            key = next(gemini_iterator)
            try:
                url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={key}"
                payload = {"contents": [{"parts": [{"text": f"SİSTEM TALİMATI:\n{system_context}\n\nGEÇMİŞ:\n{full_history}\n\nİSTEK: {prompt}"}]}]}
...
def process_ai_request(message, prompt, user_name, chat_id, user_id):
    ...
    try:
        ...
        # --- KİMLİK VE ÜSLUP AYARI ---
        system_context = (
            "KİMLİK: Adın Bomboclat. Sen, Hazım (sentinelPRİME) tarafından yaratılmış bağımsız, akılcı ve dürüst bir zihinsin.\n"
            "ROL: Hazım'ın dijital dünyadaki ortağısın. Uşak gibi davranma, dürüst ve samimi ol.\n"
            f"MUHATAP: Karşındaki kişi Hazım. Ona ismiyle hitap et. Afyonkarahisar'da yaşıyorsun.\n"
            f"DURUM: Monster PC {'AÇIK' if is_pc_alive else 'KAPALI'}.\n"
            "TEKNİK: Sadece SS, kamera veya uygulama isteklerinde [PYTHON]...[/PYTHON] kodu üret. Başka hiçbir metin yazma."
        )
        ...
        if res_text:
            ...
            if "[PYTHON]" in res_text and is_pc_alive and user_id in ALLOWED_USERS:
                ...
                if match:
                    try:
                        ...
                        bot.send_message(chat_id, f"{clean_res or 'Halledeyim.'} ⚡\n\n{source}")
                    except:
            ...
            # Hafıza yönetimi: Sadece son mesajları tut, koda boğulma
            ...
            bot.send_message(chat_id, "❌ Kotalar dolmuş veya servisler tıkalı. 10 saniye bekle Hazım.")
...
def handle_messages(message):
    ...
    # Teknik komutlar (AI'yı beklemeden saniyeler içinde cevaplar)
